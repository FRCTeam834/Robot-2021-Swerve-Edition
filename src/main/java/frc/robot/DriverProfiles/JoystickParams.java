...
public class JoystickParams {
    ...
    public void setJoystickParams(
            double deadzone, JoystickOutputTypes outputType, double rampConst) {
        ...
        if (outputType == JoystickOutputTypes.ZEROED_QUAD_LINEAR) {
            ...
            // Compute the joystick crossover threshold (expensive to compute, therefore should only
            // be
            // done once and saved)
    ...
    /**
     * Returns the precomputed linear slope of the joysticks (only if using ZEROED_QUAD_LINEAR as
     * the output type)
     *
